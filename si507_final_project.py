import requests, pandas, sqlite3, re, os.path, base64
...
def validateEmail(email):
    ...
    -------
    string
        a string stating valid or invalid input
    '''
    regex = '^(\w|\.|\_|\-)+[@](\w|\_|\-|\.)+[.]\w{2,3}$'
...
def send_email(email_from, email_to, email_subject, email_content):
...
def takeEmail():
    ''' Take email id from the user unless a valid email id is received
    Parameters
    ----------
    None
    ...
    if is_valid == 'Invalid':
        print("Please put a valid email id")
        takeEmail()
    return email
...
def mainFunc():
    ...
    #take the search query from the user
    ...
    send_email(email_from, email_to, email_subject, email_content)
